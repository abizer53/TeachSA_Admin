...
export default function LargeProfilePicModal({ source, handler }) {
  useEffect(() => {
  ...
  return (
    <div className='modal-container fixed inset-0 flex justify-center items-center bg-black/70 z-50'>
      <div className='w-3/4 sm:w-1/2 md:w-1/3 relative'>
        {/* Close button */}
        <button
          onClick={handler}
        ...
        {/* Image */}
        <img src={source} alt="Large" className='w-full aspect-square rounded-full' />
      </div>
    </div>
  );
}
