...
import AppliedjobStatusModal from './AppliedJobStatus'
...
export default function FilterBar(props) {
    ...
    const searchHandler = async (e) => {
        ...
        if(key.trim().length > 0){
            props.handler(key,status,props.data?.current_page,props.data?.per_page)
        }else{
            props.handler(key,status,props.data?.current_page,props.data?.per_page)
        }
    }
    const filtersHandler = (val) => {
        setStatus(val)
        props.handler(keyword,val,props.data?.current_page,props.data?.per_page)
    ...
    return (
        <div className='w-full flex items-center gap-3 py-3 px-5 bg-white rounded-t-md'>
            ...
            <div onClick={statusModalHandler} className='rounded-lg cursor-pointer flex items-center gap-2 border border-solid border-[#DCDBE1] py-2 px-4 relative'>
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                ...
                {statusModal && <AppliedjobStatusModal value={status} handler={filtersHandler} />}
            </div>
        </div>
    )
}