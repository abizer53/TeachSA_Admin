...
export default function SingleUser({data}) {
  return (
    <div className='w-full flex items-center justify-between border-b border-border-nput px-5 py-3 bg-white'>
      <h6 className='text-sm font-normal text-black w-3/12'>{data?.full_name}</h6>
      {/* <h6 className='text-sm font-normal text-black w-1/6 text-center'>{data?.id}</h6> */}
      <h6 className='text-sm font-normal text-black w-3/12 break-words'>{data?.email ? data?.email : "N/A"}</h6>
      {/* <h6 className='text-sm font-normal text-black w-2/12 text-center'>{data?.phone ? data?.phone : "N/A"}</h6> */}
      {/* <h6 className='text-sm font-normal text-black w-2/12 text-center capitalize'>{data?.job_category}</h6> */}
      ...
      <Link href={`/job-seekers/${data?.id}`} className='text-sm font-normal text-black w-1/12 text-center'>View</Link>
    </div>
  )
}
